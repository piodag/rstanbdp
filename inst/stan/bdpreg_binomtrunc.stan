...
data {
  ...
  vector[N] binomError; // Inverse variance weighted average
...
model {
  ...
  Beta ~ normal(BetaMu,BetaSigma) T[BetaTruncMin,BetaTruncMax]; // prior linear coefficient for binomial error structure

  Opti ~ student_t(df,0, Alpha + Beta * binomError );
