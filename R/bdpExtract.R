...
bdpExtract <- function(stanRegr,X,Y,
                       ErrorRatio=1){
  ...
  Xhat <- dat$X+(ErrorRatio*coef.ab[2]*D/(1+ErrorRatio*coef.ab[2]^2))
  Yhat <- dat$Y-(D/(1+ErrorRatio*coef.ab[2]^2))
  ...
  if (heteroscedastic=="linear") {
    ...
    return(data.frame(X = dat$X, Y = dat$Y, avgXY = (dat$X+dat$Y)/2, diffXY = dat$Y-dat$X,
                      ...
                      OptResScaled = OptRes/Sigma))

  } else {

    return( data.frame(X = dat$X, Y = dat$Y, avgXY = avgXY, diffXY = dat$Y-dat$X,
                       Xhat=Xhat, Yhat=Yhat, Xres=Xres, Yres=Yres, OptRes = OptRes))
  }
